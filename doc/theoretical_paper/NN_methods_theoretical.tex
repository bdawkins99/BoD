...
\begin{table}[H]
	...
	\node[fill=white,xscale=0.81,yscale=0.81] at (-4.755,3.3) {\bm{$L_1$} (\textbf{Eq.}~\textbf{\ref{eq:D*}})};
	
	% max-min normalized L2 metric (column 1 - row 1)
	\node[fill=white,xscale=0.81,yscale=0.81] at (-4.755,-4.83) {\bm{$L_2$} (\textbf{Eq.}~\textbf{\ref{eq:D*}})};
...
It is important that our asymptotic estimates of sample moments for distances in high-dimensional data are accurate and precise enough to be useful when considering either simulated or real data that satisfy the appropriate assumptions. We compared our asymptotic estimates of sample moments of pairwise distances by generating random data for various dimensions $m$ and $p$ (Fig.~\ref{fig:compare_theoretical_sample_moments}). For each combination of $m$ and $p$, we generated 20 random data sets, computed an $m \times m$ distance matrix $D$, computed the average pairwise distance, and computed the standard deviation of the pairwise distances. The estimates of sample moments we show are the averages of these moments from the 20 different data sets generated for each value of $m$ and $p$. Regardless of the distance metric or data type we have considered, we see a reasonably close approximation of sample moments. We explicitly show the case of Euclidean distances in standard normal data, where we fixed $m=100$ and varied the number of features $p$. Ranging from low (Fig.~\ref{fig:compare_theoretical_sample_moments}A) to relatively high (Fig.~\ref{fig:compare_theoretical_sample_moments}D) feature dimension, we see that sample moments are well approximated by asymptotic formulas of the mean (Eq.~\ref{eq:normalEucMean}) and variance (Eq.~\ref{eq:normalEucVar}). Although the mean seems to be approximately linear with respect to attribute dimension $p$ on narrow horizontal scales, the average Euclidean distance in standard normal data grows like $\sqrt{2p}$ when $p$ is sufficiently large (Eqs.~\ref{eq:normalEucMean}). This growth is immediately apparent on wider scales of the attribute dimension $p$, but the difference between sample and theoretical moments is difficult to visualize on these larger scales. Therefore, we have chosen to limit the attribute scale to be able to see how moments differ for various numbers of attributes $p$. 

\begin{figure}[H]
	\centering
	\includegraphics[width=\textwidth]{compared_moments_normal_euclidean.pdf}
	\caption{Comparison of theoretical and sample moments of Euclidean distances in standard normal data.}\label{fig:compare_theoretical_sample_moments}
\end{figure}
