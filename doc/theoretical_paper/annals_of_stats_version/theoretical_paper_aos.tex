...
\begin{aug}
	\author{\fnms{Bryan} \snm{Dawkins}\thanksref{t1,t2,m1}\ead[label=e1]{bryan-dawkins@utulsa.edu}},
	\author{\fnms{Trang} \snm{Le}\thanksref{t3,m1,m2}\ead[label=e2]{trang-le@utulsa.edu}}
	\and
	\author{\fnms{Brett} \snm{McKinney}\thanksref{t1,m2}
...
\begin{table}[H]
	...
	\begin{adjustbox}{center}
		...
		\node[fill=white,text width=1.3cm,text height=0.5cm] at (-4.755,3.25) {};
		\node[xscale=0.81,yscale=0.81] at (-4.73,3.25) {\bm{$L_1$} (\textbf{Eq.}~\textbf{\ref{eq:D*}})};
		
		% max-min normalized L2 metric (column 1 - row 1)
		\node[fill=white,text width=1.3cm,text height=0.5cm] at (-4.755,-4.88) {};
		\node[xscale=0.81,yscale=0.81] at (-4.73,-4.88) {\bm{$L_2$} (\textbf{Eq.}~\textbf{\ref{eq:D*}})};
...
\begin{minipage}[c]{0.3\textwidth}
	\caption{Resting-state fMRI transformed subject correlation matrices. Each column corresponds to an instance (or subject) $I_j$ and each column corresponds to an ROI (or feature). The notation $\hat{A}^{(j)}_{ka}$ represents the r-to-z transformed correlation between ROIs $a$ and $k \neq a$ for instance $j$.}\label{fig:rs-fMRI_matrix}
\end{minipage}
\end{figure}
...
\begin{table}[H]
	...
	\begin{adjustbox}{center}
		...
		\node[fill=white,xscale=0.91,yscale=0.91] at (-5.025,1.52) {(\textbf{Eq.}~\textbf{\ref{eq:D_rs-fMRI}})};
		
		% max-min normalized rs-fMRI metric (column 1 - row 3)
		\node[fill=white,xscale=0.91,yscale=0.91] at (-5.025,-2.52) {(\textbf{Eq.}~\textbf{\ref{eq:max-min_diff_rs-fMRI}})};
		
		% mean (row 1)
		\node[fill=white,xscale=0.83,yscale=0.83] at (3.86,2.4) {(\textbf{\ref{eq:mu_DDistr_rs-fMRI}})};
		
		% var (row 2)
		\node[fill=white,xscale=0.83,yscale=0.83] at (4.35,0.95) {(\textbf{\ref{eq:var_DDistr_rs-fMRI}})};
		
		% mean (row 3)
		\node[fill=white,xscale=0.83,yscale=0.83] at (4.03,-0.34) {(\textbf{\ref{eq:mu_max-min_rs-fMRI}})};
		...
		% var (row 4)
		\node[fill=white,xscale=0.83,yscale=0.83] at (5.67,-2.555) {(\textbf{\ref{eq:var_max-min_rs-fMRI}})};
		
		% numerator equation (row 4)
		\node[fill=white,xscale=0.88,yscale=0.88] at (4.93,-3.78) {\ref{eq:mu_DDistr_rs-fMRI}};
		
		% denominator equation (row 5)
		\node[fill=white,xscale=0.88,yscale=0.88] at (6.13,-3.78) {\ref{eq:mean_max_rs-fMRI}};
		\end{tikzpicture}
	\end{adjustbox}
\end{table}
