...
\begin{tikzpicture}
    ...
    \node[xscale=0.85,yscale=0.85] at (5.26,-1) {\bm{$P^{\text{ctrl}}_{ak}$} $\bm{=}
    	\begin{cases}
    	\bm{\rho^\textbf{hi} + \varepsilon_{ak}} & \bm{A_{ak}=1} \\
    	\bm{\rho^\textbf{lo} + \varepsilon_{ak}} & \bm{A_{ak}=0}
    	\end{cases}$
    };
    
    \node[xscale=0.85,yscale=0.85] at (12.79,-1) {\bm{$P^{\text{case}}_{ak}$} $\bm{=}
    	\begin{cases}
    	\bm{\textcolor{black!35!green}{b^\textbf{int}} + \varepsilon_{ak}} & \bm{A_{ak}=1} \hspace{0.1cm} \bm{\&} \hspace{0.1cm} \bm{\left(i \in F \text{ or } j \in F\right)} \\
    	\bm{P^\text{ctrl}_{ak}} & \text{otherwise}
    	\end{cases}$
    };
