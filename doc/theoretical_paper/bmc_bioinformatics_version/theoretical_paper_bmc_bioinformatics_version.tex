...
\author[
addressref={aff1},
email={bryan-dawkins@utulsa.edu}
]{\inits{BD}\fnm{Bryan A} \snm{Dawkins}}
\author[
addressref={aff2},
email={trang-le@utulsa.edu}
]{\inits{TL}\fnm{Trang T} \snm{Le}}
\author[
   addressref={aff1,aff3},                   % id's of addresses, e.g. {aff1,aff2}
   corref={aff3},                       % id of corresponding address, if any
   %noteref={n1},                        % id's of article notes, if any
   email={brett.mckinney@gmail.com}   % email address
]{\inits{BM}\fnm{Brett A} \snm{McKinney}}
...
\address[id=aff1]{%                           % unique id
  \orgname{Department of Mathematics, University of Tulsa}, % university, etc
  %\street{},                     %
  \postcode{74104}                                % post or zip code
  \city{Tulsa},                                   % city
  \cny{United States}                             % country
}
\address[id=aff2]{%
  \orgname{Department of Biostatistics Epidemiology and Informatics, University of Pennsylvania},
  %\street{},
  \postcode{19104}
  \city{Philadelphia},
  \cny{United States}
...
\address[id=aff3]{%
	\orgname{Tandy School of Computer Science, University of Tulsa},
	%\street{},
	\postcode{74104}
	\city{Tulsa},
	\cny{United States}
...
%\note[id=n1]{This work was supported in part by the National Institute of Health Grant No. P20GM121312 (to BAM).} % note, connected to author
%\end{artnotes}
...
%The performance of nearest-neighbor feature selection and prediction methods depends on the metric for computing neighborhoods and the distribution type of the underlying data. The effects of the distribution and metric, as well as the presence of correlation and interactions, are reflected in the expected moments of the distribution of pairwise distances. We derive general analytical expressions for the mean and variance of pairwise distances for $L_q$ metrics for Gaussian and uniform data with $p$ attributes and $m$ instances. We use extreme value theory to derive results for metrics that are normalized by the range of each attribute (max - min). In addition to these expressions applicable to the analysis of continuous data such as gene expression, we derive similar analytical expressions for a new metric for genetic variant (categorical) data in genome-wide association studies (GWAS). The distance distributions account for minor allele frequency and transition/transversion ratio. We introduce a new metric for resting-state fMRI data that is applicable to correlation-based predictors derived from time series data. Derivations assume independent data, but empirically we also consider the effect of correlation.  
\end{abstract}
...
In this section, we prove that pairwise distances converge to a normal distribution by the Central Limit Theorem under the appropriate independence assumptions. We then derive the asymptotic distributions of several distance metrics in continuous data, GWAS data, and resting-state fMRI data. Furthermore, we compare our theoretical moment estimates to sample moments in simulated distances.

\subsection{Limit distribution for \texorpdfstring{$L_q$}{} on null data}\label{sec:notation_and_CLT}
...
\subsubsection{Asymptotic normality of pairwise distances}
...
\subsection{\texorpdfstring{$L_q$}{} metric moments for continuous data distributions}\label{sec:moment_derivations}
...
\subsubsection{Distribution of \texorpdfstring{$|\text{d}_{ij}(a)|^q = |X_{ia} - X_{ja}|^q$}{}}
...
\subsubsection{Manhattan \texorpdfstring{($L_1$)}{}}
...
\subsubsection{Euclidean \texorpdfstring{($L_2$)}{}}
...
\subsubsection{Distributions of attribute range-normalized \texorpdfstring{$L_q$}{} metric}\label{sec:extremes}
...
\subsubsection{Range-Normalized Manhattan \texorpdfstring{($q=1$)}{}}
...
\subsubsection{Range-Normalized Euclidean \texorpdfstring{($q=2$)}{}}
...
\subsection{GWAS distance distributions}\label{sec:gwas_distances}
...
\subsubsection{TiTv distance distribution}\label{sec:TiTv_distances}
...
\subsubsection{Distribution of one-dimensional projected GWAS distance onto a SNP}\label{sec:GWAS_diff}
...
\subsection{Time series correlation-based distance distribution}\label{sec:rs-fMRI_distances}
...
\subsubsection{Max-min normalized time series correlation-based distance distribution}
...
\subsubsection{One-dimensional projection of rs-fMRI distance onto a single ROI}\label{sec:rs-fMRI_diff}
...
\subsubsection{Normalized Manhattan \texorpdfstring{($q=1$)}{} for rs-fMRI}
...
\subsection{Comparison of theoretical and sample moments}
...
\section{Results}
In this section, we consider the effects of pairwise attribute correlation on the distance distributions we have considered previously. We consider Euclidean distances (Eq.~\ref{eq:D},$q=2$) on standard normal data ($\mathcal{N}(0,1)$), TiTv distances (Eq.~\ref{eq:D_TiTv}) on GWAS data, and time series correlation-based distances (Eq.~\ref{eq:D_rs-fMRI}) on rs-fMRI data. We show empirically that distance distributions become positively skewed as pairwise attribute correlation increases in magnitude and discuss potential implications for nearest-neighbor feature selection methods.

\subsection{Effects of correlation on distances}\label{sec:correlation}
...
\subsubsection{Time-series derived correlation-based datasets}
...
GWAS: genome-wide associations study; MRI:magnetic resonance imaging; rs-fMRI: resting-state functional magnetic resonance imaging; fMRI: functional magnetic resonance imaging; NPDR: nearest-neighbor projected distance regression; EVT: extreme value theory; ROI: region of interest; CLT: central limit theorem; CCLT: classical central limit theorem; MGF: moment generating function; SNP: single nucleotide polymorphism; GM: genotype mismatch; AM: allele mismatch; TiTv: transition-transversion; DNA: deoxyribonucleic acid; A: adenine; T: thymine; C: cytosine; G: guanine; MAF: minor allele frequency; Ti: transition; Tv: transversion; PuPu: purine-purine; PuPy: purine-pyrimidine; PyPy: pyrimidine-pyrimidine; SD: standard deviation; ts-corr: time series-correlation
...
\subsection*{Funding}
  This work was supported in part by the National Institute of Health Grant No. P20GM121312 (to BAM). The funders had no role in study design, analysis, decision to publish, or preparation of the manuscript.

\subsection*{Author's contributions}
  BAD, TTL, and BAM conceived the project. BAD carried out all mathematical derivations. All authors read and approved the final manuscript.
...
\begin{figure}[h!]
	%\centering
	%\includegraphics[width=0.95\textwidth]{fig1_central_limit_hist_uniform-data.pdf}
	\caption{\csentence{Gaussian convergence of Manhattan and Euclidean distances.} 
...
\begin{figure}[h!]
	...
	%	\includegraphics[width=0.98\textwidth]{fig3_TiTv_distance-vs-maf.pdf}
	%\end{minipage}\hfill
	%\begin{minipage}[c]{0.31\textwidth}
		\caption{\csentence{Predicted TiTv distance versus average MAF.} 
...
\begin{table}[h!]
	...
	\begin{adjustbox}{center}
		...
		\node at (0,0) {\includegraphics[clip,trim=0.27cm 0.0cm 0.0cm 0.05cm,width=\textwidth]{tab1_updated_distributions_table(5-23-2019).pdf}};
		
		% standard Lq metric (column 1 - row 1)
		\node[fill=white,text width=1.1cm,text height=0.2cm] at (-5.49,3.45) {};
		\node[fill=white,xscale=0.83,yscale=0.83] at (-5.49,3.45) {(\textbf{Eq.}~\textbf{\ref{eq:D}})};
		
		% max-min normalized Lq metric (column 1 - row 2)
		\node[fill=white,text width=1.1cm,text height=0.2cm] at (-5.49,-3.6) {};
		\node[fill=white,xscale=0.83,yscale=0.83] at (-5.49,-3.6) {(\textbf{Eq.}~\textbf{\ref{eq:D*}})};
		
		% mean (row 1)
		\node[fill=white,xscale=0.95,yscale=0.95] at (3.84,5.6) {(\textbf{\ref{eq:normalDqMean}})};
		
		% var (row 2)
		\node[fill=white,xscale=0.95,yscale=0.95] at (5.53,4.4) {(\textbf{\ref{eq:normalVar}})};
		
		% mean (row 3)
		\node[fill=white,xscale=0.95,yscale=0.95] at (3.81,2.85) {(\textbf{\ref{eq:uniformDqMean}})};
		
		% var (row 4)
		\node[fill=white,xscale=0.95,yscale=0.95] at (5.74,1.75) {(\textbf{\ref{eq:uniformDqVar}})};
		
		% mean (row 5)
		\node[fill=white,xscale=0.95,yscale=0.95] at (3.25,0.3) {(\textbf{\ref{eq:max-min_DDistr_normal}})};
		
		% numerator mean (row 5)
		\node[fill=white,xscale=0.84,yscale=0.84] at (3.42,-0.62) {\ref{eq:normalDqMean}};
		
		% denominator mean (row 5)
		\node[fill=white,xscale=0.84,yscale=0.84] at (4.35,-0.62) {\ref{eq:med_max_normal}};
		
		% var (row 6)
		\node[fill=white,xscale=0.95,yscale=0.95] at (4.1,-1.8) {(\textbf{\ref{eq:max-min_DDistr_normal}})};
		
		% numerator mean (row 6)
		\node[fill=white,xscale=0.84,yscale=0.84] at (3.42,-2.81) {\ref{eq:normalDqMean}};
		
		% denominator mean (row 6)
		\node[fill=white,xscale=0.84,yscale=0.84] at (4.34,-2.81) {\ref{eq:med_max_normal}};
		
		% mean (row 7)
		\node[fill=white,xscale=0.95,yscale=0.95] at (3.32,-4) {(\textbf{\ref{eq:max-min_DDistr_uniform}})};
		
		% numerator mean (row 7)
		\node[fill=white,xscale=0.84,yscale=0.84] at (3.63,-4.695) {\ref{eq:uniformDqMean}};
		
		% var (row 8)
		\node[fill=white,xscale=0.95,yscale=0.95] at (3.81,-5.87) {(\textbf{\ref{eq:max-min_DDistr_uniform}})};
		
		% numerator var (row 8)
		\node[fill=white,xscale=0.84,yscale=0.84] at (3.62,-6.56) {\ref{eq:uniformDqVar}};
		\end{tikzpicture}
	\end{adjustbox}
\end{table}
...
\begin{table}[h!]
	...
	\begin{adjustbox}{center}
		\begin{tikzpicture}
		% trim=left botm right top
		\node at (0,0) {\includegraphics[clip,trim=0.27cm 0.0cm 0.0cm 0.05cm,width=0.7\textwidth]{tab2_updated_typical_metrics_table.pdf}};
		...
		% mean (row 1)
		\node[fill=white,xscale=0.95,yscale=0.95] at (3.2,3.8) {(\textbf{\ref{eq:normalManMean}})};
		
		% var (row 2)
		\node[fill=white,xscale=0.95,yscale=0.95] at (3.58,2.58) {(\textbf{\ref{eq:normalManVar}})};
		
		% mean (row 3)
		\node[fill=white,xscale=0.95,yscale=0.95] at (3.06,1.36) {(\textbf{\ref{eq:uniformManMean}})};
		
		% var (row 4)
		\node[fill=white,xscale=0.95,yscale=0.95] at (3.13,0.123) {(\textbf{\ref{eq:uniformManVar}})};
		
		% mean (row 5)
		\node[fill=white,xscale=0.95,yscale=0.95] at (3.46,-1.1) {(\textbf{\ref{eq:normalEucMean}})};
		
		% var (row 6)
		\node[fill=white,xscale=0.95,yscale=0.95] at (3.03,-2.33) {(\textbf{\ref{eq:normalEucVar}})};
		
		% mean (row 7)
		\node[fill=white,xscale=0.95,yscale=0.95] at (3.76,-3.56) {(\textbf{\ref{eq:uniformEucMean}})};
		
		% var (row 8)
		\node[fill=white,xscale=0.95,yscale=0.95] at (3.2,-4.78) {(\textbf{\ref{eq:uniformEucVar}})};
		\end{tikzpicture}
	\end{adjustbox}
\end{table}
...
\begin{table}[h!]
	...
	\begin{adjustbox}{center}
		\begin{tikzpicture}
		% trim=left botm right top
		\node at (0,0) {\includegraphics[clip,trim=0.27cm 0.0cm 0.0cm 0.05cm,width=0.85\textwidth]{tab3_updated_typical_metrics_table(normalized).pdf}};
		...
		% mean (row 3)
		\node[fill=white,xscale=0.8,yscale=0.8] at (3.53,1.45) {(\textbf{\ref{eq:max-min_mean_uniform_manhattan}})};
...
\begin{table}[h!]
	...
	\begin{adjustbox}{center}
		\begin{tikzpicture}
		% trim=left botm right top
		\node at (0,0) {\includegraphics[clip,trim=0.27cm 0.0cm 0.0cm 0.05cm,width=0.99\textwidth]{tab4_updated_distributions_table-gwas(5-23-2019).pdf}};
		
		% GM metric (column 1 - row 1)
		\node[fill=white,text width=1.5cm,text height=0.3cm] at (-5.2,3.53) {};
		\node[fill=white,xscale=0.81,yscale=0.81] at (-5.1,3.53) {(\textbf{Eq.}~\textbf{\ref{eq:D_GM}})};
		
		% AM metric (column 1 - row 2)
		\node[fill=white,text width=1.5cm,text height=0.3cm] at (-5.2,0.18) {};
		\node[fill=white,xscale=0.81,yscale=0.81] at (-5.1,0.18) {(\textbf{Eq.}~\textbf{\ref{eq:D_AM}})};
		
		% TiTv metric (column 1 - row 3)
		\node[fill=white,text width=1.5cm,text height=0.3cm] at (-5.2,-4.08) {};
		...
		% var (row 6)
		\node[fill=white,xscale=0.78,yscale=0.78] at (5.72,-4.1) {(\textbf{\ref{eq:var_DDistr_TiTv}})};
		\end{tikzpicture}
	\end{adjustbox}
\end{table}
...
\begin{table}[h!]
	...
	\begin{adjustbox}{center}
		\begin{tikzpicture}
		% trim=left botm right top
		\node at (0,0) {\includegraphics[clip,trim=0.27cm 0.0cm 0.0cm 0.05cm,width=\textwidth]{tab5_updated_distributions_table-rs-fMRI.pdf}};
		
		% standard rs-fMRI metric (column 1 - row 2)
		\node[fill=white,text width=1.5cm,text height=0.3cm] at (-4.78,1.42) {};
		...
		\node[fill=white,text width=1.5cm,text height=0.3cm] at (-4.78,-2.42) {};
		\node[fill=white,xscale=0.91,yscale=0.91] at (-4.78,-2.42) {(\textbf{Eq.}~\textbf{\ref{eq:max-min_diff_rs-fMRI}})};
		
		% mean (row 1)
		\node[fill=white,xscale=0.95,yscale=0.95] at (3.66,2.25) {(\textbf{\ref{eq:mu_DDistr_rs-fMRI}})};
		
		% var (row 2)
		\node[fill=white,xscale=0.95,yscale=0.95] at (4.12,0.9) {(\textbf{\ref{eq:var_DDistr_rs-fMRI}})};
		
		% mean (row 3)
		\node[fill=white,xscale=0.95,yscale=0.95] at (3.8,-0.33) {(\textbf{\ref{eq:mu_max-min_rs-fMRI}})};
		
		% numerator equation (row 3)
		\node[fill=white,xscale=0.84,yscale=0.84] at (4.66,-1.26) {\ref{eq:mu_DDistr_rs-fMRI}};
		
		% denominator equation (row 3)
		\node[fill=white,xscale=0.84,yscale=0.84] at (5.83,-1.26) {\ref{eq:mean_max_rs-fMRI}};
		
		% var (row 4)
		\node[fill=white,xscale=0.95,yscale=0.95] at (5.35,-2.445) {(\textbf{\ref{eq:var_max-min_rs-fMRI}})};
		
		% numerator equation (row 4)
		\node[fill=white,xscale=0.84,yscale=0.84] at (4.68,-3.59) {\ref{eq:mu_DDistr_rs-fMRI}};
		
		% denominator equation (row 5)
		\node[fill=white,xscale=0.84,yscale=0.84] at (5.83,-3.59) {\ref{eq:mean_max_rs-fMRI}};
		\end{tikzpicture}
	\end{adjustbox}
\end{table}
...
\section*{Additional Files}
  \subsection*{Additional file 1 --- Supplementary figures}
    Contains figures regarding Gaussian convergence, theoretical versus simulated moment comparison, and density curves for one-dimensional projected distances onto a single attribute.
