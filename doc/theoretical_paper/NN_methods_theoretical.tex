...
\begin{aligned}
f_{Z^q}\left(z^q_a\right) &= \frac{\frac{2}{1}}{\left(2^1\right)^{1/1}\Gamma\left(\frac{\frac{1}{1}}{\frac{2}{1}}\right)}\left(z^1_a\right)^{1/1 - 1} e^{-\left(\frac{z^1_a}{2^1}\right)^{2/1}}, \quad z_a > 0\\
...
\begin{aligned}
\mu_{Z^1} &= \frac{2^1 \Gamma\left(\frac{1 + 1}{2}\right)}{\sqrt{\pi}} \\
&= \frac{2}{\sqrt{\pi}}.
\end{aligned}
\end{equation}
...
\begin{aligned}
\sigma^2_{Z^1} &= 4^1\left[\frac{\Gamma\left(1 + \frac{1}{2}\right)}{\sqrt{\pi}} - \frac{\Gamma^2\left(\frac{1}{2}\cdot 1 + \frac{1}{2}\right)}{\pi}\right] \\
&= \frac{2(\pi-2)}{\pi}.
\end{aligned}
\end{equation}

These last few results (Eqs.~\ref{eq:projected_distance_density_normal1}-\ref{eq:1DnormalD1Var}) provide us with the null distribution for NPDR predictors when the data is from the standard normal distribution. This assumption is reasonable in the context of real data because we typically transform $X^{(m \times p)}$ so that each row has zero mean and unit variance. In addition to standardizing, simple log-transformations make gene expression data approximately satisfy the normality assumption.
