...
Suppose that $X_{ia}, X_{ja} \overset{iid}{\sim} \mathcal{F}_X(\mu_X,\sigma^2_X)$ for two fixed and distinct instances $i,j \in \{n\}^m_{n=1}$ and a fixed attribute $a \in \mathcal{A}$. 
...
Consider the smooth functional transformation $g(\alpha) = \sqrt{\alpha}$ applied to the random variable given by Eq. \ref{eq:SquEuclideanAsympt}. The Delta Method (cite textbook) can be applied to show that
