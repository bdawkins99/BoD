...
Let $F_Z$ denote the distribution function of the random variable $Z_a$. Furthermore, let $E^{(1)}=\{X_{ia}-X_{ja} \leq z_a | X_{ia} \geq X_{ja}\}$ and $E^{(2)}=\{-X_{ia}+X_{ja} \leq z_a | X_{ia} < X_{ja}\}$. Then we have

\begin{equation}\label{eq:manhattanCDF}
\begin{aligned}
F_Z(z_a) &= \text{P}[Z_a \leq z_a] \\
&= \text{P}[|X_{ia} - X_{ja}| \leq z_a] \\
&= \text{P}[\{X_{ia}-X_{ja} \leq z_a | X_{ia} \geq X_{ja}\} \cup \{-X_{ia}+X_{ja} \leq z_a | X_{ia} < X_{ja}\}] \\
&= \text{P}[E^{(1)} \cup E^{(2)}] \\
&= \text{P}[E^{(1)}] + \text{P}[E^{(2)}] - \text{P}[E^{(1)} \cap E^{(2)}] \\
&= \text{P}[E^{(1)}] + \text{P}[E^{(2)}] \\
...
It follows directly from the result in Eq. \ref{eq:manhattanCDF} that the density function of the random variable $Z_a$ is given by

\begin{equation}\label{eq:manhattanPDF}
