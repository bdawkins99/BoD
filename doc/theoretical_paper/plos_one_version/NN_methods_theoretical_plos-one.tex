...
In Section~\ref{sec:notation_and_CLT}, we introduce preliminary notation and apply the Central Limit Theorem (CLT) and the Delta Method to derive asymptotics for pairwise distances. In Section~\ref{sec:moment_derivations}, we present general derivations for continuously distributed data sets with $m$ instances and $p$ attributes. Using our more general results, we then consider the special cases of standard normal ($\mathcal{N}(0,1)$) and standard uniform ($\mathcal{U}(0,1)$) data distributions, for which we derive analytical expressions parameterized by any metric $q$, number of attributes $p$, and number of instances $m$. We focus on standard normal data because after typical pre-processing pipelines and normalization, a wide array of real data sets fall within an approximately normal distributional family. The continuous uniform distribution has applications in data simulation, Bayesian statistics when there is limited information about a random variable, and also feature selection involving lead-time in inventory problems. In Section~\ref{sec:extremes} we use Extreme Value Theory (EVT) to derive attribute range-normalized (max-min) versions of $L_q$ metrics. In Section~\ref{sec:gwas_distances}, we extend the derivations to categorical data with a binomial distribution for GWAS data with multiple metric types. In Section~\ref{sec:rs-fMRI_distances}, we present a new time series correlation-based distance metric, with a particular emphasis on rs-fMRI data, and we derive the corresponding asymptotic distance distribution results. Lastly, in Section~\ref{sec:correlation}, we demonstrate the effect of correlation in the attribute space on distance distributional properties. 
...
\end{equation}
which is typically Manhattan ($q=1$) in Relief-based methods but may also be Euclidean ($q=2$). We use the terms ``feature" and ``attribute" interchangeably for the remainder of this work. The metric 
% $\text{d}^{\text{type}}_{ij}(a)$, 
$\text{d}_{ij}(a)$,
known as a ``$\text{diff}$'' in Relief literature, can be viewed as the one-dimensional projection of the distance $D^{(q)}_{ij}$ onto a single attribute dimension $a \in \mathcal{A}$. The 
...
Our more concise d$_{ij}(a)$ notation is more commonly used in mathematical and statistical literature than the $\text{diff}(a,(i,j))$ notation that is standard in Relief-based algorithms, and it is a more universal representation of metrics in general.
NPDR does not require the division by attribute range ($\max(a)-\min(a)$) as in the case of Relief-based algorithms to constrain scores to the interval from $-1$ to $1$, where $\max(a) = \max_{k \in \mathcal{I}}\{X_{ka}\}$ and $\min(a) = \min_{k \in \mathcal{I}}\{X_{ka}\}$. The diff metric d$_{ij}(a)$ is just the magnitude difference between instance $i,j \in \mathcal{I}$ values with respect to a single attribute $a \in \mathcal{A}$. This one-dimensional projection can take on a multitude of formulations depending on the data distribution and various experimental characteristics. 
