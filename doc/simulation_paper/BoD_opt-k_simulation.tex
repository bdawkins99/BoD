...
% \begin{flushleft}
{\Large
\textbf\newline{Attribute-wise optimized k with nearest-neighbor projected-distance regression (AWOK-NPDR) for detecting interaction and main effects in high dimensional data: Blessings of Dimensionality and Adaptablity} % Please use "sentence case" for title and headings (capitalize only the first word in a title (or heading), the first word in a subtitle (or subheading), and any proper nouns).
}
%\newline
...
\begin{minipage}[c]{0.48\textwidth}
	\centering
	\begin{tikzpicture}
	\node at (-3.11,-2.45) {\fontfamily{phv}\selectfont \textbf{A}};
	\node at (0,0) {\includegraphics[width=\textwidth]{NPDR-score_vs_k.pdf}};
	\end{tikzpicture}
\end{minipage}\hfill
\begin{minipage}[c]{0.48\textwidth}
	\centering
	\begin{tikzpicture}
	\node at (-3.11,-2.45) {\fontfamily{phv}\selectfont \textbf{B}};
	\node at (0,0) {\includegraphics[width=\textwidth]{auPRC_vs_k.pdf}};
	\end{tikzpicture}
\end{minipage}
...
\begin{figure}[!bht]
	...
	\node[fill=white,xscale=0.8,yscale=0.8] at (-4.55,-3.73) {\textbf{Interaction}};
	\node[fill=white,xscale=0.8,yscale=0.8] at (-4.55,-4.1) {\textbf{(AWOK)}};
	
	\node[fill=white,text width=2cm,text height=0.2cm] at (-2.6,-3.73) {};
	\node[fill=white,xscale=0.8,yscale=0.8] at (-2.6,-3.73) {\textbf{Main}};
	\node[fill=white,xscale=0.8,yscale=0.8] at (-2.6,-4.1) {\textbf{(AWOK)}};
