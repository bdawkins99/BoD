...
\right].
\end{equation}
A diagram outlining the interaction simulation algorithm for a data set consisting of 7 features is shown in Fig.~\ref{fig:interaction_simulation_diagram}. Boxes 1 and 2 display the random network and its characteristics, such as, adjacencies and node degrees. In particular, boxes 1 and 2 show the features that are selected to be functional (highlighted in \textcolor{black!50!green}{green}). Box 3 shows the case and control correlation matrices generated using $\rho^\text{hi}$, $\rho^\text{lo}$, and $b^\text{int}$. High (\textcolor{red}{red}) and low (\textcolor{blue!70}{blue}) correlations show the functional pairwise correlations for controls and cases, respectively.

\begin{figure}[H]
\centering
\begin{minipage}[h]{1\textwidth}
\framebox{\includegraphics[width=1.0\textwidth]{interaction_simulation_method_diagram2.pdf}}
\end{minipage}

\begin{minipage}[h]{1\textwidth}
	\noindent\refstepcounter{figure}\textbf{Fig \thefigure \label{fig:interaction_simulation_diagram}.} Algorithm for interaction simulations from a random undirected network with seven nodes (or features). \textbf{Step 1:} A random network is generated, whose degree distribution is either Erd\H{o}s-R\'{e}nyi or Scale-free. \textbf{Step 2:} Adjacency matrix ($A$) and degree vector ($v_d$) corresponding to the random network are computed and functional features ($F$) are randomly selected from those with positive degree. \textbf{Step 3:} Two correlation matrices are generated for cases and controls. In the control group, high ($\rho^\text{hi}$) and low ($\rho^\text{lo}$) correlations are assigned to connected ($A_{ij}=1$) and non-connected ($A_{ij}=0$) feature pairs, respectively. In the case group, differential correlation ($b^\text{int}$) is applied to functional connections. \textbf{Step 4:} Upper triangular Cholesky factors are computed for case/control correlation matrices. \textbf{Step 5:} Standard normal random data matrices ($X^\text{ctrl}$ and $X^\text{case}$) are given correlation structure associated with case and control groups and combined into full data matrix with interaction effects ($X$).
\end{minipage}
\end{figure}
