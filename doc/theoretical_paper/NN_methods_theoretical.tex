...
Assuming data is standard normal, we substitute $q=1$ into Eq.~\ref{eq:normalPDF} to arrive at the following density function
%
...
The mean corresponding to this Generalized Gamma density is computed by substituting $q=1$ into Eq.~\ref{eq:1DnormalDqMean}. This result is given by
%
...
If we have standard uniform data, we substitute $q=1$ into Eq.~\ref{eq:uniformDqPDF} to obtain the following density function
%
...
The mean corresponding to this Kumaraswamy density is computed by substituting $q=1$ into Eq.~\ref{eq:1DuniformDqMean}. After substitution, we have the following result
%
...
Substituting $q=1$ into Eq.~\ref{eq:1DuniformDqVar} for the variance, we have the following
%
...
In the event that the data distribution is standard uniform, Eqs.~\ref{eq:projected_distance_density_uniform1}-\ref{eq:1DuniformD1Var} provide the distribution for NPDR predictors. The means (Eqs.~\ref{eq:1DnormalD1Mean} and \ref{eq:1DuniformD1Mean}) and variances (Eqs.\ref{eq:1DnormalD1Var} and \ref{eq:1DuniformD1Var}) come from the exact distribution of pairwise distances with respect to a single attribute $a \in \mathcal{A}$. This is the distribution of the so-called ``projection" of the pairwise distance onto a single attribute to which we have been referring, which is a direct implication from our more general derivations. In a similar manner, one can substitute any value of $q \geq 2$ into Eqs.~\ref{eq:normalPDF} and \ref{eq:uniformDqPDF} to derive the associated density of $Z^q_a = |X_{ia} - X_{ja}|^q$ for the given data type.
...
This concludes our analysis with continuous data distributions and the standard $L_q$ metric. In the next section, we will use extreme value theory to derive the distribution of the sample maximum and minimum for standard normal and standard uniform data. This will lead us to asymptotics for the max-min normalized $L_q$ metric used frequently in Relief-based algorithms \cite{urbanowicz17,robnik2003} for scoring features.
...
Using the first (Eq.~\ref{eq:mu_max}) and second (Eq.~\ref{eq:mu2_max}) of the distribution of sample maximum, the variance is given by
...
Using the first (Eq.~\ref{eq:mu_min}) and second (Eq.~\ref{eq:mu2_min}) of the distribution of sample minimum, the variance is given by
%
...
With the densities of attribute maximum (Eq.~\ref{eq:mu_max}) and minimum (Eq.~\ref{eq:mu_min}) for sample size $m$, the expected range is given by the following
%
...
Let $\mu_{D^{(q)}_{ij}}$ and $\sigma^2_{D^{(q)}_{ij}}$ denote the mean and variance (Eq.~\ref{eq:DDistr}) of the standard $L_q$ distance metric (Eq.~\ref{eq:D}). Then the expected value of the max-min normalized distance (Eq.~\ref{eq:D*}) distribution is given by the following
%
...
\end{equation}
%
where $m$ is the size of the sample from which extremes are derived, $\mu^{(1)}_\text{max}(m)$ is the expected value of the sample maximum (Eq.~\ref{eq:mu_max}), $\mu^{(1)}_\text{min}$ is the expected value of the sample minimum, $\mu_{D^{(q)}_{ij}}$ is the expected value of the standard $L_q$ distance metric (Eq.~\ref{eq:DDistr}), and $\sigma^2_{D^{(q)}_{ij}}$ is the variance of the standard $L_q$ distance metric (Eq.~\ref{eq:DDistr}).
...
\end{equation}
%
where $m$ is the size of the sample from which the maximum is derived, $\mu_{D^{(q)}_{ij}}$ is the expected value of the standard $L_q$ distance metric (Eq.~\ref{eq:DDistr}), and $\mu^{(1)}_\text{max}(m)$ is the expected value of the sample maximum (Eq.~\ref{eq:mu_max}).
...
\end{equation}
%
where $m$ is the size of the sample from which extremes are derived, $\mu^{(1)}_\text{max}(m)$ is the expected value of the sample maximum (Eq.~\ref{eq:mu_max}), $\sigma^2_\text{max}(m)$ is the variance of the sample maximum (Eq.~\ref{eq:sig_max}), and $\sigma^2_{D^{(q)}_{ij}}$ is the variance of the standard $L_q$ distance metric (Eq.~\ref{eq:DDistr}).
...
\end{equation}
%
where $m$ is the size of the sample from which extremes are derived, $\mu^{(1)}_\text{max}(m)$ is the expected value of the sample maximum (Eq.~\ref{eq:mu_max}), $\sigma^2_\text{max}(m)$ is the variance of the sample maximum (Eq.~\ref{eq:sig_max}),  $\mu_{D^{(q)}_{ij}}$ is the expected value of the standard $L_q$ distance metric (Eq.~\ref{eq:DDistr}), and $\sigma^2_{D^{(q)}_{ij}}$ is the variance of the standard $L_q$ distance metric (Eq.~\ref{eq:DDistr}). 
...
\end{equation}
%
where $\Phi$ and $\phi$ are the cumulative distribution function and density function, respecively, of the standard normal distribution.

The inequalities given above (Eq.~\ref{eq:mills}) show that $\frac{\phi(x)}{x \Phi(-x)} \rightarrow 1$ as $x \rightarrow \infty$. This implies that $\frac{\phi(c_m)}{c_m \Phi(-c_m)} \rightarrow 1$ as $m \rightarrow \infty$ since $c_m = -\Phi^{-1}\left(\frac{1}{m}\right) \rightarrow \infty$ as $m \rightarrow \infty$. This gives us the following approximation of the right-hand side of the expansion (Eq.~\ref{eq:log_expand}) given previously
%
...
\end{equation}
%
where $m$ is the size of the sample from which the maximum is derived and $\gamma$ is the Euler-Mascheroni constant. The median of this distribution is given by
%
...
\end{equation}
%
where $m$ is the size of the sample from which th maximum is derived and $\gamma$ is the Euler-Mascheroni constant.
...
\end{equation}
%
where $m$ is the size of the sample from which extremes are derived, $\mu_{D^{(q)}_{ij}}$ is the expected value (Eq.~\ref{eq:uniformDqMean}) of the $L_q$ metric (Eq.~\ref{eq:D}) in standard uniform data, and $\sigma^2_{D^{(q)}_{ij}}$ is the variance (Eq.~\ref{eq:uniformDqVar}) of the $L_q$ metric (Eq.~\ref{eq:D}) in standard uniform data.
...
\end{equation}
%
where $\mu^{(1)}_\text{max}(m)$ is the expected attribute maximum (Eq.~\ref{eq:med_max_normal}), $m$ is the size of the sample from which the maximum is derived, and $p$ is the total number of attributes. Similar to the variance of the standard Manhattan distance, the variance of the max-min normalized Manhattan distance is on the order of $p$ for fixed instance dimension $m$. For fixed $p$, the variance (Eq.~\ref{eq:max-min_var_normal_manhattan}) vanishes as $m$ grows without bound. If we fix $m$, the same variance increases monotonically with increasing $p$.
...
\end{equation}
%
where $m$ is the size of the sample from which extremes are derived and $p$ is the total number of attributes. Interestingly, the variance of the max-min normalized Manhattan distance in standard uniform data approaches $p/18$ as $m$ increases without bound for a fixed number of features $p$. This is the same asymptotic value to which the variance of the standard Manhattan distance (Eq.~\ref{eq:uniformManMean}) converges. Therefore, large sample sizes make the variance of the normalized Manhattan distance approach the variance of the standard Manhattan distance in standard uniform data.
...
\end{equation}
%
where $m$ is the size of the sample from which extremes are derived. Similar to the variance of max-min normalized Manhattan distances in standard uniform data (Eq.~\ref{eq:max-min_var_uniform_manhattan}), the variance of normalized Euclidean distances approaches the variance of the standard Euclidean distances in uniform data (Eq.~\ref{eq:uniformEucVar}) as $m$ increases without bound. That is, the variance of the max-min normalized Euclidean distance (Eq.~\ref{eq:max-min_var_uniform_euclidean}) approaches $7/120$ as $m$ grows larger.

\begin{table}[H]
	\caption{Summary of distance distribution derivations for standard normal and standard uniform data. Asymptotic estimates are given for both standard (Eq.~\ref{eq:D}) and max-min normalized (Eq.~\ref{eq:D*}) q-metrics. These estimates are relevant for all $q \in \mathbb{N}$ and $p$ for which the normality assumption of distances holds.}
	...
	\node[fill=white,xscale=0.83,yscale=0.83] at (-5.79,3.9) {(\textbf{Eq.}~\textbf{\ref{eq:D}})};
	
	% max-min normalized Lq metric (column 1 - row 2)
	\node[fill=white,xscale=0.83,yscale=0.83] at (-5.79,-3.6) {(\textbf{Eq.}~\textbf{\ref{eq:D*}})};
	
	% mean (row 1)
	\node[fill=white,xscale=0.8,yscale=0.8] at (4.05,6) {(\textbf{\ref{eq:normalDqMean}})};
	
	% var (row 2)
	\node[fill=white,xscale=0.8,yscale=0.8] at (5.73,4.8) {(\textbf{\ref{eq:normalVar}})};
	
	% mean (row 3)
	\node[fill=white,xscale=0.8,yscale=0.8] at (4.03,3.15) {(\textbf{\ref{eq:uniformDqMean}})};
	
	% var (row 4)
	\node[fill=white,xscale=0.8,yscale=0.8] at (5.98,2) {(\textbf{\ref{eq:uniformDqVar}})};
	
	% mean (row 5)
	\node[fill=white,xscale=0.8,yscale=0.8] at (3.5,0.5) {(\textbf{\ref{eq:max-min_DDistr_normal}})};
	
	% numerator mean (row 5)
	\node[fill=white,xscale=0.81,yscale=0.81] at (3.6,-0.55) {\ref{eq:normalDqMean}};
	
	% denominator mean (row 5)
	\node[fill=white,xscale=0.81,yscale=0.81] at (4.55,-0.55) {\ref{eq:med_max_normal}};
	
	% var (row 6)
	\node[fill=white,xscale=0.8,yscale=0.8] at (4.4,-1.9) {(\textbf{\ref{eq:max-min_DDistr_normal}})};
	
	% numerator mean (row 6)
	\node[fill=white,xscale=0.81,yscale=0.81] at (3.6,-3.035) {\ref{eq:normalDqMean}};
	
	% denominator mean (row 6)
	\node[fill=white,xscale=0.81,yscale=0.81] at (4.55,-3.035) {\ref{eq:med_max_normal}};
	
	% mean (row 7)
	\node[fill=white,xscale=0.8,yscale=0.8] at (3.58,-4.33) {(\textbf{\ref{eq:max-min_DDistr_uniform}})};
	
    % numerator mean (row 7)
	\node[fill=white,xscale=0.81,yscale=0.81] at (3.8,-5.05) {\ref{eq:uniformDqMean}};
	
	% var (row 8)
	\node[fill=white,xscale=0.8,yscale=0.8] at (4.05,-6.384) {(\textbf{\ref{eq:max-min_DDistr_uniform}})};
	
	% numerator var (row 8)
	\node[fill=white,xscale=0.81,yscale=0.81] at (3.8,-7.1) {\ref{eq:uniformDqVar}};
	\end{tikzpicture}
	\end{adjustbox}
\end{table}

\begin{table}[H]
	\caption{Asymptotic estimates of means and variances for the standard $L_1$ and $L_2$ distance distributions. Estimates for both standard normal and standard uniform data are given.}
	...
	% standard  Lq metric (column 1 - row 1)
	\node[fill=white,xscale=0.83,yscale=0.83] at (-3.57,1.93) {\bm{$L_1$} (\textbf{Eq.}~\textbf{\ref{eq:D}})};
	
	% standard Lq metric (column 1 - row 2)
	\node[fill=white,xscale=0.83,yscale=0.83] at (-3.57,-3.25) {\bm{$L_2$} (\textbf{Eq.}~\textbf{\ref{eq:D}})};
	
	% mean (row 1)
	\node[fill=white,xscale=0.8,yscale=0.8] at (3.4,4) {(\textbf{\ref{eq:normalManMean}})};
	
	% var (row 2)
	\node[fill=white,xscale=0.8,yscale=0.8] at (3.78,2.705) {(\textbf{\ref{eq:normalManVar}})};
	
	% mean (row 3)
	\node[fill=white,xscale=0.8,yscale=0.8] at (3.26,1.413) {(\textbf{\ref{eq:uniformManMean}})};
	
    % var (row 4)
	\node[fill=white,xscale=0.8,yscale=0.8] at (3.33,0.123) {(\textbf{\ref{eq:uniformManVar}})};
	
	% mean (row 5)
	\node[fill=white,xscale=0.8,yscale=0.8] at (3.66,-1.15) {(\textbf{\ref{eq:normalEucMean}})};
	
    % var (row 6)
	\node[fill=white,xscale=0.8,yscale=0.8] at (3.2,-2.46) {(\textbf{\ref{eq:normalEucVar}})};
	
	% mean (row 7)
	\node[fill=white,xscale=0.8,yscale=0.8] at (3.96,-3.76) {(\textbf{\ref{eq:uniformEucMean}})};
	
	% var (row 8)
	\node[fill=white,xscale=0.8,yscale=0.8] at (3.4,-5.05) {(\textbf{\ref{eq:uniformEucVar}})};
	\end{tikzpicture}
	\end{adjustbox}
\end{table}

\begin{table}[H]
	\caption{Asymptotic estimates of means and variances for the max-min normalized $L_1$ and $L_2$ distance distributions. Estimates for both standard normal and standard uniform data are given. The cumulative distribution function of the standard normal distribution is represented by $\Phi$. Furthermore, $\mu^{(1)}_\text{max}(m)$ (Eq.~\ref{eq:med_max_normal}) is the asymptotic median of the sample maximum from $m$ standard normal random samples.}
	...
	\node[fill=white,text width=1.5cm,text height=0.5cm] at (-4.755,3.3) {};
	\node[xscale=0.81,yscale=0.81] at (-4.73,3.3) {\bm{$L_1$} (\textbf{Eq.}~\textbf{\ref{eq:D*}})};
	
	% max-min normalized L2 metric (column 1 - row 1)
	\node[fill=white,text width=1.5cm,text height=0.5cm] at (-4.755,-4.83) {};
	\node[xscale=0.81,yscale=0.81] at (-4.73,-4.83) {\bm{$L_2$} (\textbf{Eq.}~\textbf{\ref{eq:D*}})};
	
	% mean (row 1)
	\node[fill=white,xscale=0.78,yscale=0.78] at (3.95,7.08) {(\textbf{\ref{eq:max-min_mean_normal_manhattan}})};
	
	% var (row 2)
	\node[fill=white,xscale=0.8,yscale=0.8] at (5.08,4.343) {(\textbf{\ref{eq:max-min_var_normal_manhattan}})};
	
	% mean (row 3)
	\node[fill=white,xscale=0.8,yscale=0.8,text width=0.8cm] at (3.73,1.5) {(\textbf{\ref{eq:max-min_mean_uniform_manhattan}})};
	
	% var (row 4)
	\node[fill=white,xscale=0.8,yscale=0.8] at (4.24,0.2) {(\textbf{\ref{eq:max-min_mean_uniform_manhattan}})};
	
	% mean (row 5)
	\node[fill=white,xscale=0.8,yscale=0.8] at (3.98,-1.125) {(\textbf{\ref{eq:max-min_mean_normal_euclidean}})};
	
	% var (row 6)
	\node[fill=white,xscale=0.8,yscale=0.8] at (4.88,-3.825) {(\textbf{\ref{eq:max-min_var_normal_euclidean}})};
	
	% mean (row 7)
	\node[fill=white,xscale=0.8,yscale=0.8] at (4.6,-6.685) {(\textbf{\ref{eq:max-min_mean_uniform_euclidean}})};
	
	% var (row 8)
	\node[fill=white,xscale=0.8,yscale=0.8] at (4.245,-7.985) {(\textbf{\ref{eq:max-min_var_uniform_euclidean}})};
	\end{tikzpicture}
	\end{adjustbox}
\end{table}
...
\begin{figure}[H]
	...
	\caption{Density curves and moments of TiTv distance as a function of average MAF $\bar{f}_a$, given by Eq.~\ref{eq:avg_maf}, and Ti/Tv ratio $\eta$, given by Eq.~\ref{eq:TiTv_constraints2}. (\textbf{A}) For fixed $\bar{f}_a=0.055$, TiTv distance density is plotted as a function of increasing $\eta = 0.5, 1, 1.5, \text{ and } 2$. TiTv distance decreases as $\eta$, the Ti/Tv ratio, increases. For $\eta=\text{Ti/Tv}=0.5$, there are twice as many transversions as there are transitions. On the other hand, $\eta=\text{Ti/Tv}=2$ indicates that there are half as many transversions as there are transitions. Since transversions encode a larger magnitude distance than transitions in Eq.~\ref{eq:diff_TiTv}, this behavior is expected. (\textbf{B}) Simulated and predicted mean $\pm$ SD are shown as a function of increasing Ti/Tv ratio $\eta$. Distance decreases as Tv becomes more frequent than Ti. Theoretical and simulated moments are approximately the same. (\textbf{C}) For fixed $\eta=2$, TiTv distance density is plotted as a function of increasing $\bar{f}_a = 0.055, 0.150, 0.250, \text{ and } 0.350$. TiTv distance increases as $\bar{f}_a$ approaches a maximum of 0.5, which means that there is about the same frequency of minor alleles as primary alleles at locus $a$. (\textbf{D}) Simulated and predicted mean $\pm$ SD are shown as a function of increasing average MAF $\bar{f}_a$. Distance increases as the number of minor alleles increases at each locus $a$. Theoretical and simulated moments are approximately the same.}\label{fig:TiTv_ridge}
\end{figure}
...
\begin{table}[H]
	...
	\node[fill=white,xscale=0.81,yscale=0.81] at (-5.44,3.73) {(\textbf{Eq.}~\textbf{\ref{eq:D_GM}})};
	
	% AM metric (column 1 - row 2)
	\node[fill=white,xscale=0.81,yscale=0.81] at (-5.44,0.22) {(\textbf{Eq.}~\textbf{\ref{eq:D_AM}})};
	
	% TiTv metric (column 1 - row 3)
	\node[fill=white,xscale=0.81,yscale=0.81] at (-5.44,-4.28) {(\textbf{Eq.}~\textbf{\ref{eq:D_TiTv}})};
	
	% mean (row 1)
	\node[fill=white,xscale=0.78,yscale=0.78] at (3.5,5.21) {(\textbf{\ref{eq:mu_DDistr_GM}})};
	
	% var (row 2)
	\node[fill=white,xscale=0.78,yscale=0.78] at (4.35,3.57) {(\textbf{\ref{eq:var_DDistr_GM}})};
	
	% mean (row 3)
	\node[fill=white,xscale=0.78,yscale=0.78] at (3.5,1.94) {(\textbf{\ref{eq:mu_DDistr_AM}})};
	
	% var (row 4)
	\node[fill=white,xscale=0.78,yscale=0.78] at (4.45,0.28) {(\textbf{\ref{eq:var_DDistr_AM}})};
	
	% mean (row 5)
	\node[fill=white,xscale=0.78,yscale=0.78] at (6.04,-2) {(\textbf{\ref{eq:mu_DDistr_TiTv}})};
	
	% var (row 6)
	\node[fill=white,xscale=0.78,yscale=0.78] at (6.08,-4.313) {(\textbf{\ref{eq:var_DDistr_TiTv}})};
	\end{tikzpicture}
	\end{adjustbox}
\end{table}
...
\end{equation}
where $A^{(i)}_{ka}$ and $A^{(j)}_{ka}$ are the correlations between ROI $a$ and ROI $k$ for instances $i$ and $j$, respectively. With this rs-fMRI diff, the pairwise distance between two instances $i,j \in \mathcal{I}$ is computed as follows
%
\begin{equation}\label{eq:D_rs-fMRI}
D^\text{fMRI}_{ij} = \sum_{a \in \mathcal{A}} \text{d}^\text{ROI}_{ij}(a).
\end{equation}

In order for comparisons between different correlations to be possible, we first perform a Fisher r-to-z transform on the correlations. We then load all of the transformed correlations into a $p(p-1) \times m$ matrix $X$ (see Fig.~\ref{fig:rs-fMRI_matrix}).
...
\begin{minipage}[c]{0.3\textwidth}
	\captionsetup{type=figure}\captionof{figure}{Resting-state fMRI transformed subject correlation matrices. Each column corresponds to an instance (or subject) $I_j$ and each column corresponds to an ROI (or feature). The notation $\hat{A}^{(j)}_{ka}$ represents the r-to-z transformed correlation between ROIs $a$ and $k \neq a$ for instance $j$.}\label{fig:rs-fMRI_matrix}
\end{minipage}
...
The coefficient estimates found through least squares fitting are $\beta_0 = - \beta_1 \approx 0.08$. These estimates allow us to arrive at a functional form for the double sum in the right-hand side of Eq. \ref{eq:var_DDistr_rs-fMRI} that is proportional to $\frac{2p(\pi-2)(p-1)}{\pi}$. That is, we have the following formula for approximating the double sum
%
\begin{equation}\label{eq:estimate_cov_form}
2\sum_{a = 1}^{p-1} \sum_{r=a+1}^{p-1} \text{Cov}\left(\sum_{k=a+1}^{p} 2\left|\hat{A}^{(i)}_{ak} - \hat{A}^{(j)}_{ak}\right|, \sum_{s=r+1}^{p} 2\left|\hat{A}^{(i)}_{rs} - \hat{A}^{(j)}_{rs}\right|\right) = \frac{p(\pi - 2)(p - 1)}{4\pi}.
\end{equation}
...
\begin{table}[H]
	...
	\node[fill=white,xscale=0.91,yscale=0.91] at (-5.025,1.58) {(\textbf{Eq.}~\textbf{\ref{eq:D_rs-fMRI}})};
	
	% max-min normalized rs-fMRI metric (column 1 - row 3)
	\node[fill=white,xscale=0.91,yscale=0.91] at (-5.025,-2.48) {(\textbf{Eq.}~\textbf{\ref{eq:max-min_diff_rs-fMRI}})};
	
	% mean (row 1)
	\node[fill=white,xscale=0.83,yscale=0.83] at (3.86,2.48) {(\textbf{\ref{eq:mu_DDistr_rs-fMRI}})};
	
	% var (row 2)
	\node[fill=white,xscale=0.83,yscale=0.83] at (4.35,1.07) {(\textbf{\ref{eq:var_DDistr_rs-fMRI}})};
	
	% mean (row 3)
	\node[fill=white,xscale=0.83,yscale=0.83] at (4.13,-0.22) {(\textbf{\ref{eq:mu_max-min_rs-fMRI}})};
	
	% numerator equation (row 3)
	\node[fill=white,xscale=0.88,yscale=0.88] at (4.93,-1.32) {\ref{eq:mu_DDistr_rs-fMRI}};
	
	% denominator equation (row 3)
	\node[fill=white,xscale=0.88,yscale=0.88] at (6.13,-1.32) {\ref{eq:mean_max_rs-fMRI}};
	
	% var (row 4)
	\node[fill=white,xscale=0.83,yscale=0.83] at (5.74,-2.555) {(\textbf{\ref{eq:var_max-min_rs-fMRI}})};
	
	% numerator equation (row 4)
	\node[fill=white,xscale=0.88,yscale=0.88] at (4.93,-3.873) {\ref{eq:mu_DDistr_rs-fMRI}};
	
	% denominator equation (row 5)
	\node[fill=white,xscale=0.88,yscale=0.88] at (6.13,-3.873) {\ref{eq:mean_max_rs-fMRI}};
	\end{tikzpicture}
	\end{adjustbox}
\end{table}
