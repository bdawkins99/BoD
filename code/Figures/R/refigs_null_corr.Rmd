...
for(iter in 1:n_iters){
  ...
  sim_dfs <- rbind(sim_dfs, data.frame(sim_df, r = mean.cor.corr))
...
```{r}
joy_6 <- sim_dfs %>%
  mutate(TypeFct = fct_rev(as.factor(Type)),
         rFct = fct_rev(as.factor(paste('r = ', round(r, 3))))) %>%
  ggplot(aes(y = rFct, fill = TypeFct)) +
  # geom_density(alpha = 0.5) +
  ggridges::geom_density_ridges(
    aes(x = Value, fill = paste(TypeFct)), 
    scale = 2, alpha = .8, color = 'white'
  ) +
    ggridges::theme_ridges(grid = F) +
    scale_fill_viridis_d() +
    labs(x = NULL, y = NULL, caption = 'Euclidean distance') +
...
joy_7 <- sim_dfs %>%
  ...
  ggridges::geom_density_ridges(
    aes(x = Value, fill = paste(TypeFct)), 
    scale = 2, alpha = .8, color = 'white', from = 0, to = 100
  ) +
  ggridges::theme_ridges(grid = F) +
  scale_fill_viridis_d() +
  labs(x = NULL, y = NULL, caption = 'TiTv distance') +
  theme(legend.position = 'None',
        legend.title = element_blank(),
        plot.margin = unit(c(0, 0, 0, 0.15), 'cm'),         
        plot.caption = element_text(hjust=0.4),
...
# joy_7
# joy_7 %>%
#   ggsave(filename = 'code/Figures/R/re_fig_7b.pdf', width = 5, height = 4)
```
...
joy_8 <- sim_dfs %>%
  ...
  ggridges::theme_ridges(grid = F) +
  scale_fill_viridis_d() +
  labs(x = NULL, y = NULL, caption = 'rs-fMRI distance') +
  theme(legend.position = c(0.46, 0.95),
        legend.title = element_blank(),
        plot.margin = unit(c(0, 0, 0, 0.15), 'cm'),
        plot.caption = element_text(hjust=0.4),
        axis.text.y = element_text(margin = margin(t = 0, r = -7, b = 0, l = 0))) +
  guides(fill = guide_legend(reverse = TRUE)) +
  scale_x_continuous(limits = c(780, 1350)) +
  scale_y_discrete(expand = c(0.01, 0)) 
# joy_8
# joy_8 %>%
#   ggsave(filename = 'code/Figures/R/re_fig_8b.pdf', width = 5, height = 4)
```


```{r}
my_title <- cowplot::ggdraw() + 
  cowplot::draw_label('Correlated and uncorrelated distances') 
my_plots <- cowplot::plot_grid(joy_6, joy_7, joy_8, labels = 'AUTO', ncol = 3)
 
my_title %>%
  cowplot::plot_grid(my_plots, ncol = 1, rel_heights=c(0.1, 1)) %>%
  ggsave(filename = 'code/Figures/R/null_corr.pdf', width = 10, height = 3.5 )
```
