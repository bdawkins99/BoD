...
for(mixed.iter in 1:length(mixed.ratio)){
  ...
  for(iter in 1:num.iter){
    ...
    pct.imbalance <- 0.5
    main.bias <- 0.4
    interaction.bias <- 0.5
    label <- "class"
    sim.type <- "mainEffect" # main effect simulations
    mix.type <- "mainEffect-interactionErdos"
    ...
    data.sets <- createSimulation2(num.samples=n.samples,
    ...
    for(i in 1:ncv_folds[1]){
      ...
      for(k.iter in 1:length(ks)){
        ...
        npdr.cc.results <- npdr(trn.pheno, trn.data, regression.type="binomial", attr.diff.type="numeric-abs",
                                nbd.method="relieff", nbd.metric = "manhattan", msurf.sd.frac=.5, knn=k,
                                neighbor.sampling="none",
                                dopar.nn = F, padj.method="bonferroni", verbose=verbose)
        ...
        for(j in 1:ncv_folds[2]){
          ...
          trn.data.inner <- as.matrix(trn.data[inner_idx, npdr.positives])
          tst.data.inner <- as.matrix(trn.data[-inner_idx, npdr.positives])
      ...
      fold_atts_list[[i]] <- npdr.positives
      
      #print(ncol(trn.data[,stir.positives]))
      rf.model.tmp <- randomForest(trn.data[,npdr.positives], y=trn.pheno, mtry=max(floor(ncol(trn.data[,npdr.positives])/3),1),ntree=num_tree)
      pred.class <- predict(rf.model.tmp,tst.data[,npdr.positives])
    ...
    if(iter == num.iter){
      ...
      pdf(paste("NPDR_pct_mixed",mixed.ratio[mixed.iter],"_test_accu-vs-k.pdf",sep=""),height=7,width=7)
    ...
    npdr.cc.results <- npdr(train.pheno, train.data, regression.type="binomial", attr.diff.type="numeric-abs",
                            nbd.method="relieff", nbd.metric = "manhattan", msurf.sd.frac=.5, knn=best_k,
                            neighbor.sampling="none",
                            dopar.nn = F, padj.method="bonferroni", verbose=verbose)
    #t_sorted_stir <- results.list$STIR_T[,-3]  # remove cohen-d
    #t_sorted_stir <- results.list$STIR_T[,-3]  # remove cohen-d
    ...
    positive.stats <- attrDetectStats(functional,npdr.positives)
  ...
  write.csv(data.frame(chosen.k=chosen.k),file=paste("NPDR_chosen_k_pct-mixed_",mixed.ratio[mixed.iter],".csv",sep=""),row.names=F)
  write.csv(data.frame(precision=unlist(my.precision.list),recall=unlist(my.recall.list)),file=paste("NPDR_recall_precision_pct-mixed_",mixed.ratio[mixed.iter],".csv",sep=""),row.names=F)
  pdf(paste("NPDR_chosen_k_hist_pct-mixed",mixed.ratio[mixed.iter],".pdf",sep=""),height=7,width=7)
...
write.csv(out.df,file="NPDR_nestedCV-k_mixedEffects_summary.csv",row.names=F)
round(mean(chosen.k))
pdf("NPDR_hist-chosen_k-mixedEffects.pdf",height=6,width=6)
hist(chosen.k,breaks=10,freq=F)
dev.off()
