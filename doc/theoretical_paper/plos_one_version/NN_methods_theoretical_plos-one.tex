...
\section{Nearest-neighbor projected-distance regression}
As in the case of preceding nearest-neighbor feature selection algorithms, the performance of NPDR relies on appropriate choice of neighborhood selection criteria. The size of neighborhoods must be chosen appropriately for optimal detection of important statistical effects. It has been shown using simulations that neighborhood order should be as large as possible to optimally detect main effects, whereas smaller neighborhoods are necessary for detecting interactions~\cite{mckinney13}. NPDR allows for any neighborhood algorithm to be used, such as fixed or adaptive $k$, and fixed or adaptive radius. Especially in the case of fixed or adaptive radius methods, one needs some sense of central tendency with respect to pairwise distances between a given target instance and its neighbors. Choosing a radius that is too large will potentially yield too many neighbors if the goal of feature selection is finding interaction effects. On the other hand, small radii will inhibit the detection of main effects. Another problem exists for choosing a fixed $k$ number of neighbors. We would like to choose $k$ so that the average distance within neighborhoods is not too large or too small with respect to the empirical average pairwise distance between pairs of instances. In order for the appropriate choice of neighborhood size to be made, we need to know the central tendency and scale of our distance distribution.
