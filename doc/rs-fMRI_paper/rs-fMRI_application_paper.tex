...
% \begin{flushleft}
{\Large
\textbf\newline{Nearest-neighbor projected distance regression feature selection for resting-state fMRI study of major depressive disorder using multiple brain atlases} % Please use "sentence case" for title and headings (capitalize only the first word in a title (or heading), the first word in a subtitle (or subheading), and any proper nouns).
}
%\newline
% Insert author names, affiliations and corresponding author email (do not include titles, positions, or degrees).
\begin{center}
  \begin{tabular}{l}
  Bryan A. Dawkins$^{\text{1}}$, LIBR collaborators$^{\text{2}}$, and Brett A. McKinney$^{\text{1,3,}*}$ \\
...
\section*{Abstract}
Nearest-neighbor projected distance regression (NPDR) is a new machine learning feature selection algorithm that uses metrics to detect interaction effects between predictor variables that affect disease outcomes. In a resting-state functional MRI (rs-fMRI) study, the subject level data consists of correlation matrices between the time series from brain Regions of Interest (ROIs). We present new metrics for NPDR tailored to correlation-based features so that feature selection can be targeted at the ROI level. An additional challenge of rs-fMRI data is the variety of parcellations of the brain or atlases that may lead to differences in brain region importance in feature selection. We use integer programming to derive a mapping between brain atlases to determine spatially similar ROIs. We apply the new NPDR to calculate relative importance of ROIs for classification of major depression versus healthy controls in two existing brain atlases. With ROI importance scores and spatial similarity between atlases, we present a novel modality for the comparison of brain atlases and the selection of ROIs relevant to major depression.
...
\section{Methods}
In this section, we first provide a brief description of NPDR in the context of dichotomous outcomes in resting-state fMRI. We then present our method of deriving a one-to-one mapping between the ROIs in two fMRI brain atlases, which we use to compare and contrast the spatial distribution of ROIs in a pair of atlases. Furthermore, we describe real rs-fMRI data generated on healthy controls (HC) and subjects with major depressive disorder (MDD) that we analyze in this work. In the next section, we use NPDR to rank ROIs in each atlas with respect to their relevance in classifying the binary MDD-HC outcome, which will allow us to identify important brain regions and functional networks applicable to MDD.
...
Using our attribute diff (Eq.~\ref{eq:diff_rs-fMRI}), we compute NPDR standardized beta coefficients of the generalized linear model given by
%
...
We employ an adaptive-$k$ algorithm, previously referred to as Gene-Wise Adaptive $k$ (GWAK)~\cite{mckinney13} for gene expression data, that we call Variable-Wise Optimized $k$ (VWOK) to determine the optimal $k$ for each ROI in the two atlases we are analyzing. Although developed for gene expression data, this method can easily be used for any data type to which nearest-neighbor feature selection is applied. VWOK uses a greedy approach to optimize the relevance score for each attribute as a function of $k$ (Fig.~\ref{fig:gwak}). For each attribute $a \in \mathcal{A}$, a relevance score is computed for each value of $k=1,2,\dots,m-1$ using some nearest-neighbor feature selection algorithm. Each attribute is assigned the value of $k$ that maximizes its relevance score. For noise attributes, scores do not change significantly as a function of $k$. On the other hand, the scores of relevant attributes will change significantly as a function of $k$. This implies that VWOK maximizes the probability of detecting relevant attributes when nearest-neighbor feature selection is utilized. 

We have determined empirically with simulated data that cross-validation approaches for tuning $k$ to optimally select relevant features do not perform well in nearest-neighbor feature selection as compared to VWOK. This is due largely to the fact that cross-validation optimizes test classification accuracy, which does not necessarily give the highest quality features. Partitioning data into training and test folds also limits the choice of $k$ because the training data has only a subset of available instances in the full data set. Partitioning the data into training and test sets to choose a single value of $k$ for the entire data set will cause the selection of a suboptimal $k$ for a given relevant attribute, which could be detrimental to the quality of selected features. Our goal is to rank features in terms of relevance to a phenotype of interest as accurately as possible using a nearest-neighbor approach. Therefore, we use VWOK in our current analysis.

\begin{figure}[h!]
	\centering
	\framebox{\includegraphics[width=0.8\textwidth]{feature_wise_opt_diagram3.pdf}}
	\caption{Variable-Wise Optimized $k$ (VWOK) method for choosing optimal values of $k$ in nearest-neighbor feature selection. For each attribute $a \in \mathcal{A}$ and each value of $k=1,2,\dots,m-1$, a score or weight is computed with a nearest-neighbor feature selection method like Relief-Based Algorithms (RBAs) or another similar method. Each feature is assigned the value of $k$ that maximizes its importance score. Winning scores are then sorted in decreasing order and some fraction of the top scoring attributes are chosen for further analysis. We use the adjusted pseudo P values from NPDR to filter out attributes that are determined irrelevant to the phenotype. In our current context, attribute scores are simply the NPDR standardized beta coefficients computed for the corresponding GLM (Eq.~\ref{eq:glm_roi}).}\label{fig:gwak}
\end{figure}
